...
#| echo: false
#| output: asis
# load code coverage and test results
test_files_rds <- list.files(file.path(params$input_dir), pattern = "_covr_and_test_results.Rds", full.names = TRUE)

# attempt loading session info details
session_info_txt <- list.files(file.path(params$input_dir), pattern = "^session", full.names = TRUE)
...
# if a session_info_* file was found, then load the details here:
if (length(session_info_txt) == 1) {
  cat("<br />### Session information")
  paste0("> ", readLines(session_info_txt), collapse = "\n") |>
    cat()
}
```
